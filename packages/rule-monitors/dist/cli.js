...
class RuleCLI {
    ...
    async loadConfig() {
        try {
            ...
            for (const [monitorName, monitorConfig] of Object.entries(config.monitors || {})) {
                this.system.updateMonitorConfig(monitorName, monitorConfig);
            }
    ...
    generateMarkdownReport(result) {
        ...
        if (result.violations.length > 0) {
            report += `## Violations\n\n`;
            // Group by category
            const violationsByCategory = result.violations.reduce((acc, violation) => {
...
<body>
    ...
    async autoFix(result) {
        const autoFixableViolations = result.violations.filter((v) => v.auto_fixable);
        ...
        for (const violation of autoFixableViolations) {
            try {
                ...
                console.error(`❌ Failed to fix: ${violation.message} - ${error.message}`);
            }
        }
    }

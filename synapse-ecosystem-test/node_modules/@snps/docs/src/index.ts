  ...
  public async render(template: string, data: Record<string, any>): Promise<string> {
    ...
    result = result.replace(/\{%\s*for\s+(\w+)\s+in\s+([^%]+)\s*%\}([\s\S]*?)\{%\s*endfor\s*%\}/g, (match, itemVar, arrayPath, content) => {
      ...
      return array.map((item: any) => {
        ...
        itemContent = itemContent.replace(/\{%\s*if\s+([^%]+)\s*%\}([\s\S]*?)\{%\s*endif\s*%\}/g, (condMatch: string, condition: string, condContent: string) => {
          ...
          if (condition.includes('==')) {
            ...
            if (leftValue === rightValue) {
              ...
              processedContent = processedContent.replace(/\{%\s*for\s+(\w+)\s+in\s+([^%]+)\s*%\}([\s\S]*?)\{%\s*endfor\s*%\}/g, (nestedMatch: string, nestedItemVar: string, nestedArrayPath: string, nestedContent: string) => {
                ...
                return nestedArray.map((nestedItem: any) => {
                  ...
                  nestedItemContent = nestedItemContent.replace(/\{\{(\w+)\}\}/g, (nestedMatch: string, key: string) => {
              ...
              processedContent = processedContent.replace(/\{\{(\w+)\}\}/g, (varMatch: string, key: string) => {
        ...
        itemContent = itemContent.replace(/\{%\s*for\s+(\w+)\s+in\s+([^%]+)\s*%\}([\s\S]*?)\{%\s*endfor\s*%\}/g, (nestedMatch: string, nestedItemVar: string, nestedArrayPath: string, nestedContent: string) => {
          const nestedArray = item[nestedArrayPath] || [];
        ...
        itemContent = itemContent.replace(/\{\{(\w+)\}\}/g, (itemMatch: string, key: string) => {
          const value = item[key] || '';
